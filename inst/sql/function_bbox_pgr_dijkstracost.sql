...
create
	or replace function openroads.bbox_pgr_dijkstracost ( sql text, start_pt geometry, end_pt geometry, directed boolean, tol_dist float8 default 100, expand_percent float4 default 0.5 ) returns table ( agg_cost float8 ) as $body$ declare
...
-- around the bbox of start and end
	var_expand_geom := st_expand ( st_makeline ( start_pt, end_pt ), st_distance ( start_pt, end_pt ) * expand_percent );
